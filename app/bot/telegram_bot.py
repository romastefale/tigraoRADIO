...
async def _with_db(callable_fn: Callable[[Any], Any]) -> Any:
    def _run() -> Any:
...
def _render_track(track: dict[str, Any], detailed: bool) -> str:
    if not detailed:
...
def _rate_guard(message: Message) -> bool:
    ...
    if user is None:
        return False
    return rate_limiter.allow(user.id)
...
async def cmd_start(message: Message) -> None:
    ...
    await message.answer(
        "👋 Hi! Spotify helper bot.\n"
        "Use: /login /play /album /artist /ranking"
    )
...
async def cmd_login(message: Message) -> None:
    if not await _send_rate_limited(message):
        return
    await message.answer("🔐 Login: open your backend URL and visit /spotify/login")
